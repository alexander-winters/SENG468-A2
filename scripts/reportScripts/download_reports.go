...
func DownloadReports(username string) {
	urls := []string{
		fmt.Sprintf("http://localhost/reports/user/%s/posts", username),
		fmt.Sprintf("http://localhost/reports/user/%s/comments", username),
		fmt.Sprintf("http://localhost/reports/user/%s/likes", username),
	}
